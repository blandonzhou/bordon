...
<!--
	$(function(){
		...
		$("#cache").formValidator({onshow:"<?php echo L("enter_the_cache_input_will_not_be_cached")?>",onfocus:"<?php echo L("enter_the_cache_input_will_not_be_cached")?>",empty:true}).regexValidator({regexp:"num1",datatype:'enum',param:'i',onerror:"<?php echo L("cache_time_can_only_be_positive")?>"}).defaultPassed();
		$("#num").formValidator({onshow:"<?php echo L('input').L("num")?>",onfocus:"<?php echo L('input').L("num")?>",empty:true}).regexValidator({regexp:"num1",datatype:'enum',param:'i',onerror:"<?php echo L('that_shows_only_positive_numbers')?>"}).defaultPassed();
...
<div class="pad-10">
<form action="?m=tag&c=tag&a=edit&id=<?php echo $id?>" method="post" id="myform">
<div>
<fieldset>
  ...
  <?php if ($module):?>
    <tr>
	  ...
	  <?php if(isset($html[$action]) && is_array($html[$action]) && $action)foreach($html[$action] as $k=>$v):?>
		...
		<td class="y-bg"><?php echo creat_form($k, $v, $form_data[$k], $module)?></td>
	</tr>
