...
<form name="myform" action="?m=video&c=video&a=<?php echo $_GET['a']?>" method="post" id="myform" enctype="multipart/form-data" ><input type="hidden" name="userupload" value="1">
<table width="100%" class="table_form">

<tr>
	<td width="120"><?php echo L('select_upload_channel');?></td> 
		<td>
			<label class="ib cu" style="width:125px">
			<input type="radio" name="channelid" value='1'  /> 
			<?php echo L('upload_to_ku6');?> 
			</label> 
			
			<label class="ib cu" style="width:125px">
			<input type="radio" name="channelid" value = "3" checked > 
			<?php echo L('upload_to_local');?> 
			</label> 
			
			
			<?php if($flash_info['allow_upload_to_vms']) {?>
			<label class="ib cu" style="width:125px">
			<input type="radio" name="channelid" value='2'> 
			<?php echo L('upload_to_ku6vms');?> </label><?php }?>
		</td>
...
<script type="text/javascript" src="<?php echo JS_PATH;?>plupload/plupload.full.min.js"></script>
	<tr id="local_method">
		...
		<td>
		<div id="container">
			<input type='text' name='bc_file' id='local_video' value='' class='input-text' style='width:20%'/>
			<input id='pickfiles' type='button' class='button' value='选择视频'>
			<input id='uploadfiles' type='button' class='button' value='上传视频'>		
		</div>
		</td>
	</tr>
...
$("input[name='channelid']").bind('click',function(){
	var v = $(this).val();
	
	if(v == 3){
		$("#ku6_method").css('display','none');
		$("#local_method").css('display','table-row');
	}else{
	$("#local_method").css('display','none');
		$("#ku6_method").css('display','table-row');
...
<script type="text/javascript" src="<?php echo JS_PATH;?>plupload/base.js"></script>	


	
	<tr id = "ku6_method" style="display:none">
		<td width="120"><?php echo L('choose_videoes');?></td> 
