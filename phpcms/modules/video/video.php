...
class video extends admin {
	...
	public function add() {
		if ($_POST['dosubmit']) {
			...
			if( !empty($_POST['bc_file']) ){

				$data = array(
						'userupload' =>$_POST['userupload'],
						'channelid' =>$_POST['channelid'],
						'picpath' => $_POST['bc_file'],
						'title' => $_POST['title'],
					 ...
					 if(extension_loaded('ffmpeg')){
						...
						$thumb_arr = explode('.', basename($_POST['bc_file']) );
						$thumb = $thumb_arr[0];
					
							 //判断ffmpeg是否载入     
						$mov = new ffmpeg_movie(APP_PATH . $_POST['bc_file']);//视频的路径 
						
					/* 	$ext = $thumb_arr[1];
				
						if($ext != 'avi') {
						
							$ffmpeg = 'ffmpeg.exe';//成功（当前目录）
							 
							$cmd= $ffmpeg. ' -i  '.$_POST['bc_file'].' -ab 56 -ar 22050 -b 500 -r 15 -s 320x240  uploadfile/video/' . $thumb .'.mp4';
						
					$cmd= $ffmpeg. '  -i '. $_POST['bc_file'] .' -b 360 -r 25 -s 320x240 -ab 56 -ar 22050 -ac 1 uploadfile/video/'. $thumb .'.mp4';
						...
						//生成缩略图	 
							...
							$real_path = "uploadfile/thumb/";
							 ...
							 $img = realpath($real_path) . '/' . $thumb . '.jpg';
