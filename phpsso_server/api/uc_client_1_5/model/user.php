...
class usermodel {
	...
	function add_user($username, $password, $email, $salt, $uid = 0, $questionid = '', $answer = '', $regip = '') {
	...
	function edit_user($username, $oldpw, $newpw,  $email, $salt = '', $ignoreoldpw = 0, $questionid = '', $answer = '') {
		...
		if(!$ignoreoldpw && $data['password'] != md5(md5($oldpw).$salt)) {
			return -1;
		}

		$sqladd = $newpw ? "password='".md5(md5($newpw).$salt)."', `salt`='$salt'" : '';
